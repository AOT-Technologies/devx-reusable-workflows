...
- **SonarQube analysis no longer fails a run when no token is available.**
  ...
  contributor cannot fix, and presented a missing credential as a code problem.
  The analysis is now skipped with an explicit warning explaining why, and the
